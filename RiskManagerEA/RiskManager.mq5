...
input int    InpServerOffsetHours   = 2;    // サーバー時間オフセット(UTC+2=冬時間, UTC+3=夏時間)
...
int GetJSTDay()
{
   ...
   // JSTに変換
   int jstHour = dt.hour + 9 - InpServerOffsetHours;
   if(jstHour >= 24) { dt.day += 1; }
   return dt.day;
}
...
int GetJSTTime()
{
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   int jstHour = (dt.hour + 9 - InpServerOffsetHours + 24) % 24;
   return jstHour * 100 + dt.min;
}
...
int OnInit()
{
   ...
   Print(StringFormat("RiskManager v1.1 初期化完了 | 開始残高: %.2f | 目標残高: %.2f",
                        ...
                        const MqlTradeResult&      result)
{
   ...
   if(dealProfit < 0.0)
   {
      ...
      {
         bool isNY2  = IsNYTime();
         int  limit2 = isNY2 ? InpNYMaxLosses : InpMaxLosses;
         Print(StringFormat("連敗カウント: %d / %d", g_consecutiveLosses, limit2));
      }
   }
   else
   {
...
bool IsAllowedTime()
{
   ...
   if(jstTime >= 2130)                    return true; // 21:30-24:00
   if(jstTime >= 0    && jstTime <  100)  return true; // 00:00-01:00

   return false;
}

//+------------------------------------------------------------------+
//| NY時間帯かどうかを判定（JST 21:30〜翌01:00）                    |
//+------------------------------------------------------------------+
bool IsNYTime()
{
   int jstTime = GetJSTTime();
   if(jstTime >= 2130) return true;
   if(jstTime >= 0 && jstTime < 100) return true;
   return false;
}
...
{
   double currentBalance = AccountInfoDouble(ACCOUNT_BALANCE);
   double profitPct      = (g_startBalance > 0.0) ?
   ...
   else if(IsAllowedTime())
   {
      ...
      timeStatus  = "OK エントリー許可 (" + zone + ")";
      timeColor   = clrLime;
   }
   else
   {
      timeStatus = "NG エントリー禁止時間帯";
   ...
   // ラベル描画
   DrawLabel(PREFIX + "Title",   "[ RiskManager v1.1 ]",
             10, 20, 14, clrWhite);
