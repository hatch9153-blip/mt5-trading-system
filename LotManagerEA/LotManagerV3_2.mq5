//+------------------------------------------------------------------+
//|                                          LotManagerV3_2.mq5      |
//|                   MT5 Lot Management & Grid Order EA v3.2        |
...
#property description "Lot management EA v3.2: Calc P/L 0.00-skip fix, MaxRisk/Reward display, tickValue fallback."
...
void LoadSettings()
{
   ...
   Print("LotManagerV3.2: Settings restored from GlobalVariables.");
}
...
int OnInit()
{
   ...
   Print("LotManagerV3.2 initialized on ", g_symbol);
...
void OnDeinit(const int reason)
{
   ...
   Print("LotManagerV3.2 deinitialized. Settings saved.");
}
...
//  Fix5/Fix6/Fix7: CalcExpectedPL
//  - Fix5: removed priceHigh/Low <= 0 skip; now skips only when both prices are equal AND splits > 1
//  - Fix6: tickValue/tickSize refreshed at call time if cached values are zero
//  - Fix7: returns rawRisk/rawReward (before caps) for display alongside capped values
void CalcExpectedPL(double &totalRisk, double &totalReward,
                    double &rawRisk,   double &rawReward)
{
   totalRisk = 0.0; totalReward = 0.0;
   rawRisk   = 0.0; rawReward   = 0.0;

   // Fix6: refresh tick info if cached values are zero
   double tv = g_tickValue;
   double ts = g_tickSize;
   if(tv <= 0.0) tv = SymbolInfoDouble(g_symbol, SYMBOL_TRADE_TICK_VALUE);
   if(ts <= 0.0) ts = SymbolInfoDouble(g_symbol, SYMBOL_TRADE_TICK_SIZE);
   if(tv <= 0.0 || ts <= 0.0)
   {
      Print("LotManagerV3.2: tickValue/tickSize unavailable, cannot calc P/L");
      return;
   }
   ...
   for(int z = 0; z < g_zoneCount; z++)
   {
      ...
      // Fix5: skip only when both prices are equal and splits > 1 (no range to divide)
      if(g_zones[z].priceHigh == g_zones[z].priceLow && g_zones[z].splits > 1) continue;
      ...
      for(int s = 0; s < g_zones[z].splits; s++)
      {
         ...
         if(sl > 0.0)
            zoneRisk   += MathAbs(entryPrice - sl) / ts * tv * InpBaseLot;
         if(tp > 0.0)
            zoneReward += MathAbs(tp - entryPrice)  / ts * tv * InpBaseLot;
      }

      // Accumulate raw (before per-zone caps)
      rawRisk   += zoneRisk;
      rawReward += zoneReward;
...
void PlaceGridOrders()
{
   ...
   for(int z = 0; z < g_zoneCount; z++)
   {
      ...
      string comment = "LMv32_Z" + IntegerToString(z+1);
   ...
   Print("LotManagerV3.2: Placed ", totalPlaced, " orders.");
   UpdatePanelInfo();
...
void CancelAllGridOrders()
{
   ...
   Print("LotManagerV3.2: All grid orders cancelled.");
}
...
// Fix2/Fix7: ReadPanelFields before calc; show capped and raw values
void UpdateExpectedPL()
{
   ReadPanelFields();  // Fix2: ensure latest field values are loaded

   // Fix6: also refresh tick cache here
   g_tickValue = SymbolInfoDouble(g_symbol, SYMBOL_TRADE_TICK_VALUE);
   g_tickSize  = SymbolInfoDouble(g_symbol, SYMBOL_TRADE_TICK_SIZE);

   double risk = 0.0, reward = 0.0, rawRisk = 0.0, rawReward = 0.0;
   CalcExpectedPL(risk, reward, rawRisk, rawReward);

   // Fix7: show "$capped (raw: $XX)" when a cap reduces the value
   string riskStr, rewardStr;
   bool riskCapped   = (rawRisk   > risk   + 0.001 && risk   > 0.0);
   bool rewardCapped = (rawReward > reward + 0.001 && reward > 0.0);

   if(risk == 0.0 && reward == 0.0)
   {
      riskStr   = "--- (set High/Low/SL/TP)";
      rewardStr = "---";
   }
   else
   {
      riskStr   = riskCapped   ?
                  "$" + DoubleToString(risk,   2) + "  (raw $" + DoubleToString(rawRisk,   2) + ")" :
                  "$" + DoubleToString(risk,   2);
      rewardStr = rewardCapped ?
                  "$" + DoubleToString(reward, 2) + "  (raw $" + DoubleToString(rawReward, 2) + ")" :
                  "$" + DoubleToString(reward, 2);
   }

   ObjectSetString(0, "LM_ValRisk",   OBJPROP_TEXT, riskStr);
   ObjectSetString(0, "LM_ValReward", OBJPROP_TEXT, rewardStr);
   ChartRedraw(0);
}
