...
残高比例ロット バックテスト
============================
ロット計算: floor(残高 / 10,000) × 0.01 lot
例: ¥10,000 → 0.01lot、¥20,000 → 0.02lot、¥100,000 → 0.10lot

GOLD・USDJPY 両銘柄を ¥10,000 / ¥100,000 スタートで検証
"""
...
# ロット計算: ¥10,000ごとに0.01lot（最小0.01lot）
def calc_lot(balance, unit=10_000, lot_per_unit=0.01):
    lots = int(balance / unit) * lot_per_unit
...
def run_backtest(df1m, initial_balance, symbol):
    """残高比例ロットでバックテスト実行"""
    is_gold = (symbol == 'GOLD')
    # GOLD: pnl_jpy = pnl_usd × USD_JPY × CONTRACT_SIZE × lot
    #       CONTRACT_SIZE=10, USD_JPY=150
    # USDJPY: pnl_jpy = 価格差 × 100,000 × lot
    SPREAD = 0.30 if is_gold else 0.03
    USD_JPY = 150.0

    print(f"\n{'='*65}")
    print(f"バックテスト [残高比例ロット / {symbol}]: 初期資金 ¥{initial_balance:,}")
    print(f"  初期ロット: {calc_lot(initial_balance):.2f}lot")
    print(f"{'='*65}")
    ...
    for i in range(50, n):
            ...
            if exit_signal:
                ...
                if is_gold:
                    if position_side == 'long':
                        ...
                        pnl_jpy = (entry_price - exit_price - SPREAD) * 10 * current_lot * USD_JPY
                else:
                    if position_side == 'long':
                        pnl_jpy = (exit_price - entry_price - SPREAD) * 100_000 * current_lot
                    else:
                        pnl_jpy = (entry_price - exit_price - SPREAD) * 100_000 * current_lot
        ...
        if i % 50000 == 0 and i > 0:
            print(f"  進捗: {i/n*100:.1f}% | トレード数: {len(trades)} | 残高: ¥{balance:,.0f} | lot: {calc_lot(balance):.2f}")

    # 未決済強制クローズ
    if in_position:
        last = df5m.iloc[-1]; ep = last['close']
        if is_gold:
            pnl_jpy = (ep - entry_price - SPREAD) * 10 * current_lot * USD_JPY if position_side == 'long' \
                      else (entry_price - ep - SPREAD) * 10 * current_lot * USD_JPY
        else:
            pnl_jpy = (ep - entry_price - SPREAD) * 100_000 * current_lot if position_side == 'long' \
...
if __name__ == '__main__':
    ...
    for symbol, csv_dir in [('GOLD', '/home/ubuntu/stoch_backtest/csv_data'),
        ...
        for init_bal in [10_000, 100_000]:
            trades_df, final_bal = run_backtest(df1m, init_bal, symbol)
            stats = analyze(trades_df, init_bal)
            results[symbol][init_bal] = {'trades': trades_df, 'stats': stats}

            if stats:
                s = stats
                print(f"\n--- {symbol} ¥{init_bal:,} スタート [残高比例ロット] ---")
    ...
    with open(f'{OUTPUT_DIR}/backtest_results_dynamic_lot.pkl', 'wb') as f:
        pickle.dump(results, f)
    print("\n\n結果を保存しました (backtest_results_dynamic_lot.pkl)")
