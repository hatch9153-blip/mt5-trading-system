"""
Stochastics EA バックテストエンジン ロジック② - USDJPY版
==========================================================
...
【USDJPY固有の設定】
- スプレッド: 0.03円（0.3pips）
- 損益計算: 1lot = 100,000通貨 → micro口座 0.01lot = 1,000通貨
  - 1pip = 0.01円 → 0.01lot × 1,000通貨 × 0.01円/pip = 0.1円/pip
  - pnl_jpy = (exit - entry) × 0.01lot × 100,000 = (exit - entry) × 1,000
"""
...
# ===== 設定 =====
CSV_DIR    = "/home/ubuntu/stoch_backtest/csv_usdjpy"
OUTPUT_DIR = "/home/ubuntu/stoch_backtest"
INITIAL_BALANCES = [10_000, 100_000]

# USDJPY micro口座 0.01lot
# 1lot = 100,000通貨 → 0.01lot = 1,000通貨
# pnl_jpy = (exit_price - entry_price) × 1,000
LOT_SIZE      = 0.01
CONTRACT_SIZE = 100_000   # 1lot = 100,000通貨
SPREAD_PIP    = 0.03      # スプレッド 0.03円（0.3pips）
...
def run_backtest(df1m, initial_balance):
    print(f"\n{'='*65}")
    print(f"バックテスト [ロジック② USDJPY]: 初期資金 ¥{initial_balance:,}")
    print(f"{'='*65}")
    ...
    for i in range(50, n):
        ...
        if in_position:
            ...
            if exit_signal:
                ...
                # USDJPY損益計算: pnl_jpy = (価格差) × lot × 100,000
                if position_side == 'long':
                    pnl_jpy = (exit_price - entry_price - SPREAD_PIP) * LOT_SIZE * CONTRACT_SIZE
                else:
                    pnl_jpy = (entry_price - exit_price - SPREAD_PIP) * LOT_SIZE * CONTRACT_SIZE
                balance += pnl_jpy
        ...
        if strategy_mode == 'long':
            ...
            gc_5m_entry = (pk5m <= pd5m_val and k5m_now > d5m_now and
                           k5m_now <= OVERSOLD + 5)
            if gc_5m_entry:
                entry_price   = row['close'] + SPREAD_PIP
                in_position   = True
                entry_time    = dt
                position_side = 'long'
    ...
    if in_position:
        ...
        ep = last['close']
        if position_side == 'long':
            pnl_jpy = (ep - entry_price - SPREAD_PIP) * LOT_SIZE * CONTRACT_SIZE
        else:
            pnl_jpy = (entry_price - ep - SPREAD_PIP) * LOT_SIZE * CONTRACT_SIZE
        balance += pnl_jpy
        trades.append({
            'entry_time': entry_time, 'exit_time': last['dt'],
            'side': position_side, 'entry_price': entry_price, 'exit_price': ep,
            'pnl_jpy': round(pnl_jpy, 2), 'balance': round(balance, 2),
...
if __name__ == '__main__':
    ...
    for init_bal in INITIAL_BALANCES:
        ...
        if stats:
            s = stats
            print(f"\n--- ¥{init_bal:,} スタート 結果サマリー [ロジック② USDJPY] ---")
    ...
    with open(f'{OUTPUT_DIR}/backtest_results_usdjpy.pkl', 'wb') as f:
        pickle.dump(results, f)
    print("\n結果を保存しました (backtest_results_usdjpy.pkl)")
